	...
	tests := []struct {
	...
	}{
		{
			...
			required: []string{
				...
				"e2e-postgres-backup-restore.ps1", "e2e-audit-archive.ps1", "e2e-identity-readiness.ps1", "docker compose up -d --build",
				".artifacts/postgres-recovery/", ".artifacts/audit-archive/", ".artifacts/identity-readiness/", "docker compose down --volumes --remove-orphans",
			},

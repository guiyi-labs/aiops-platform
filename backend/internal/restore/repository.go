...
type Repository interface {
	...
	ActiveBySource(context.Context, int64, string, string) (Plan, bool, error)
}
...
func (r *GormRepository) Complete(ctx context.Context, id, idempotencyKey string, executedAt time.Time, plan Plan, result *ExecutionResultJSON) (Plan, error) {
	...
	if result != nil {
		...
		updates["velero_restore_uid"] = result.RestoreUID
	}
...
func (r *GormRepository) ActiveBySource(ctx context.Context, clusterID int64, backupName, backupNamespace string) (Plan, bool, error) {
	var plan Plan
	err := r.db.WithContext(ctx).
		Where("cluster_id = ? AND source_backup_name = ? AND source_backup_namespace = ? AND status IN ?", clusterID, backupName, backupNamespace, []string{StatusAwaitingConfirmation, StatusExecuting}).
		First(&plan).Error
	if err == gorm.ErrRecordNotFound {

...
> 当前功能基线：M98/M99/M100 全部完成；M101 本地数据轨（WAL/PITR/备库/故障注入 8 场景）与 M89 身份轨本地预研（OIDC 登录 14 场景）、M102 本地轨道双全新环境安装演练（10/10）、跨 digest 升级/回滚（14/14）、第三环境备份恢复（16/16）、**可复现闭环演示（15/15）**、**最终交付文档（测试矩阵/已知限制/兼容范围/运维手册/安全声明）**与**离线安装包全链路演练（10/10，含可复用离线包发布）**、**M94 回放纳入演示闭环（17/17）并刷新 `v0.3.0-rc.5-replay` 离线包（10/10）**与 **rc.5-replay 双环境跨 digest 升级/回滚/备份恢复证据**、**回放浏览器通道 e2e 闭环（diagnosis-timeline 12/12、全量 e2e 68/68）**与 **离线包自包含化（迁移入包、不再依赖宿主机源码）**、**M103 告警→事故联动（demo-drill 告警提升事故 22/22，含后端富集/去重与前端入口）**、**M104 巡检结果→事故联动（demo-drill 巡检提升事故 28/28，含集群防泄漏与前端入口）**、**M105 信号归一化摄取 + 信号→事故联动（demo-drill 信号提升事故 32/32，含诊断 drain 接线与集群防泄漏）、**M106 本地体验重构（admin123 默认口令、登录全屏化、侧栏折叠基线、启用最新栈 v0.3.0-m106）**、**M107 事故证据时间线（incident 详情五源证据聚合 + 深链，evidence API 全链路）**已归档；M89/M90 组织授权轨保持 Deferred，版本保持 RC，不宣称 GA。

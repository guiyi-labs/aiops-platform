...
> 当前功能基线：M95 统一 Finding 与证据模型（tag `baseline-m95-finding-detail-v2-20260810`；FindingDetail v2 后端统一证据模型）
...
- **docs/changes/**：146 份变更记录 + 1 份模板。M21–M95 每个里程碑均有独立 `YYYY-MM-DD-mXX-*.md` change-record（M61–M66、M74、M75 已于 2026-08-09 补齐）；M1–M20 早期以主题文档（认证、引导、集群接入等）形式归档。
...
| M95 | 统一证据模型：`FindingDetail v2`（规则身份/证据引用/类型化建议/版本信息）、v1→v2 兼容层、共享严重度映射、按资源合并保留规则来源、golden DatasetVersion 1.2 + 迁移提示；11 posture 分析器 + finops schema parity |

> 当前执行入口：`docs/next-long-term-plan.md`。M93-B2 已关闭、M94 第三步（根因卡 + 证据时间线 + 行动区 + 深链）已落地、M95 统一证据模型（FindingDetail v2）后端已落地；继续 M95 前端统一证据组件增量与 M96–M102；

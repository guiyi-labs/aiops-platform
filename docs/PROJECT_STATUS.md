...
| M99（M99-A/B） | 信号关联：SLO burn 信号化管道（`slo.burn.*` + `SLOBurnSignalSink`）、correlation `rollout_causes_slo_burn` 规则与 11 场景黄金回放、AIOps 路由生产接线（M99-A）；metricshistory Deployment readiness 采集 + `slo.MetricshistorySource`（workload_readiness 滚动窗口求值、无数据 fail-closed）、`POST /slos` ID 修复（M99-B）；M99-C/D（关联 worker、前端展示）待续 |

...
| M100（M100-A） | 安全与租户治理：路由权限矩阵生成与差异门禁（`docs/security/permission-matrix.md`，279 路由按角色/scope/审计归档）；`/aiops` 查询维度集群/命名空间授权强制（`requireClusterQueryAccess`，无授权 viewer 探测 signals/slos/correlation 由返回数据变 404）；M100-B/C/D（会话失效旅程、敏感字段扫描、依赖/SBOM 差异）待续 |

...
> 当前功能基线：`744bf1f`（M1–M93-A + W10–W12：登录页数据真实性、确定性浏览器回归、真实集群 kind E2E、发布签名 fail-closed）
...
- **CHANGELOG.md**：覆盖 M1–M93-A 全部 `Added/Changed/Fixed` 条目（Unreleased 含 W10–W12 / M88 / M91 / M92 / M93-A）。
- **docs/changes/**：138 份变更记录 + 1 份模板。M21–M93-A 每个里程碑均有独立 `YYYY-MM-DD-mXX-*.md` change-record（M61–M66、M74、M75 已于 2026-08-09 补齐）；M1–M20 早期以主题文档（认证、引导、集群接入等）形式归档。
...
> 待办：M89 生产身份 / M90 数据可靠性需组织授权（P3）；M93-B 收口 ParticleNetwork 生命周期、动态降级和低端设备性能证据，随后推进诊断叙事和规模性能。

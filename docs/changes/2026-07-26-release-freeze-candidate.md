...
M1 基础运行、M2 多集群资源读取、M3 确定性诊断、M4 AI/审计/通知/受控处置、M5 交付包装和 M6 演示演示环境均已完成。本阶段将可运行基线整理为可复现的发布候选，并补齐交接所需的证据索引。

## 冻结动作

- 重新执行 `scripts/verify.ps1`，确认后端、前端、Compose、Kustomize 和 HTTP 健康检查仍然通过。
- 运行源码与文档敏感材料扫描，确认未发现私钥、长令牌、JWT、Bearer 凭据或明文密码。
- 保留当前演示演示环境，未清理演示集群、诊断记录和受控处置记录。
...
演示 ServiceAccount Token 仅保留约一小时。正式演示前应重新执行 `scripts/demo-up.ps1`，演示结束执行 `scripts/demo-down.ps1`。

## 交付文档

- 交接入口：`docs/development-handoff.md`
- 文档材料索引：`docs/README.md`
...
4. 根据演示或部署需要创建 release tag。

在上述确认完成前，仓库保持无初始 Git commit 的状态，不影响本地运行、验证和文档材料使用。

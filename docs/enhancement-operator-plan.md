...
- 基线: main @ `fa03612`（公众门面清理版；M115 收口完成，门禁 70% 已上调）
- 关联: 指挥中枢战略任务「Operator/CRD 增强」（K8s 云原生能力加分项 + client-go 深度证据）
...
**硬性约束**（确认时默认携带）：noreply 提交；不破坏 M115 收口、不碰本地私有材料及 ignore 守卫；不写无法验证的「生产级 / 已验证」表述；完成推送后回报 commit SHA + 验证证据。
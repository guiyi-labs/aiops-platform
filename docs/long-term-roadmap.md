...
- Status: Active（执行基线）
- Updated: 2026-08-09
- Baseline: `main` @ `48af578`（本地领先 `origin/main` 4 个提交，工作树干净）
...
本地 `main` 领先 `origin/main` 4 个提交，工作树干净，已打 tags：
...
验证状态：
- 前端全绿：typecheck / eslint / vitest（21 files · 122 tests）/ vite build。
- 后端全绿：gofmt / build / `go test ./...`；fuzz seed 冒烟 + benchmark 首刷基线。
- 全局覆盖率 55.3%（< 60% 目标，显式跟踪项，见 W8）。

---

## 1. 五条可度量基线（对齐 polish-plan）

| 维度 | 打磨目标 | 当前进度 |
|---|---|---|
| AIOps 差异化 | 聚合态势 + 端到端闭环 + 黄金回归全覆盖 | 主线（P1），M80 已起步 |
...
- 文档基线对齐实际 HEAD（README/PROJECT_STATUS 更新到 M84 后的真实状态）。
- 补齐 M1–M20 与 M61–M66 缺失的独立 change-record。
- 全局覆盖率从 55.3% 渐进拉向 ≥60%（重点：cmd、repository、store、migrations 等低覆盖层）。

### Phase 1：AIOps 差异化（主线，按月里程碑推进）

- **W5/M81 AIOps 端到端闭环**：优化中心 findings → 巡检计划（M52）→ 确定性诊断（M43）→ AI 引用解释（M55）→ dry-run 预览（M19）。让"发现问题 → 解释 → 给方案 → 预览"一条链路可点击、可回放。
- **W6/M82 黄金回归全覆盖**：把聚合态与主要分析器 findings 纳入 M56 黄金回放 + 质量报告；discovery 契约快照。
- **W7/M83 拓扑深化**：Gateway API（Gateway/HTTPRoute）只读接入；分页、折叠、聚合视图；500 节点 fixture 渲染验证。

### Phase 2：工程卓越（P2，每月 1–2 里程碑）

- **W8/M84 测试强度**：已完成核心包 ≥70% 门禁 + fuzz + benchmark；**剩余增量** 全局覆盖率 ≥60%（显式跟踪项）。
- **W9/M85 前端质量 + 浏览器 E2E**：Playwright（1280×720 + 390×844），关键链路 ≥5 条、console error=0、补 a11y 基础、bundle 体积门禁。
- **W10/M86 契约与 API 治理**：全路由错误码审计、OpenAPI 深度 diff、前端类型从 openapi.yaml 生成+同步校验。
...
- **M89 数据可靠性**：WAL 归档 + 定时 PITR 演练 + backend 多副本优雅停机 + 迁移回滚演练。
...
1. **W5/M81 AIOps 闭环串联**——差异化主线里第一个可演示的端到端链路，价值最高、无需外部授权。
2. **全局覆盖率 ≥60%**——按包渐进补测，CI 门禁从 50% 提到 60%，补齐 W8 剩余增量。
3. **W9/M85 Playwright**——工具链可离线试跑，补前端真实浏览器证据。
4. **文档基线对齐 + 补 change-record**——稳定 P0 收口。

> 依赖提示：kind E2E（涉及 W3/W5/W7 集群行为）需要 Docker/kind 运行时；当前本机 Docker daemon 未运行，相关验收证据需在推送远端并具备 kind 环境后再补。
...
- 本计划不为"功能数量"突破这些边界；只提升"可发布、可验证、可规模"三层成立度。
...
**进度（2026-08-12，本地轨道第一步）**：M102 双全新环境一致安装演练完成（`scripts/dual-env-compose-drill.sh`，两套完全隔离环境以同一不可变镜像 digest 安装并断言 install/关键旅程/数据持久化/清理一致性，10/10 PASS，报告见 `.artifacts/dual-env-compose-drill/`，记录见 `docs/changes/2026-08-12-m102-dual-env-compose-drill.md`）。跨 digest 升级/回滚仍由 CI release 生命周期覆盖，需组织环境补齐；M89/M90 未完成前版本保持 RC，不宣称 GA。

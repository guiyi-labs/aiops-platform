...
**进度（2026-08-10，M96-A/B）**：已落地 `m96-v1` 确定性 fixture 配置、流式 gzip NDJSON 生成/校验器，以及包含 P50/P95/P99、heap、goroutine、分页、取消、超时和背压的后端 report-mode 基准；后续仍需完成前端 DOM 预算、嵌套路由壳层与主题覆盖收敛，并在连续稳定周期后决定是否升级性能阈值。

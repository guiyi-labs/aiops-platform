...
**进度（2026-08-12，本地轨道 v2）**：M102 双全新环境安装/升级/回滚演练完成（`scripts/dual-env-compose-drill.sh`，两套完全隔离环境：install/关键旅程/数据持久化/清理 10/10；设置 `APP_UPGRADE_BACKEND_IMAGE` 后补跨 digest 升级（version `dev → v0.3.0-rc.5`）与回滚（version 还原），14/14 PASS，报告见 `.artifacts/dual-env-compose-drill/`，记录见 `docs/changes/2026-08-12-m102-dual-env-compose-drill.md` v2）。真实组织 kind/Helm 生命周期与备份恢复、M89/M90 授权轨仍需组织环境补齐；未完成前版本保持 RC，不宣称 GA。

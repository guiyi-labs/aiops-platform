...
### M94：诊断叙事与证据时间线（5–7 天）✅ 已收口（2026-08-12，`baseline-m94-replay-20260812`；前三步 `baseline-m94c-deep-links-20260810`）

**当前状态**：根因卡、只读证据时间线、类型化行动区、关联入口深链（资源详情/工作负载/审计）与回放模式（按事件时间重放 M81 insight 链路，只读不伪造）均已完成并归档；M94 收口。
...
**进度（2026-08-12，本地数据轨第一步）**：M101-M90 本地 WAL/PITR/备库/故障注入演练完成（`scripts/wal-pitr-drill.sh`，8 场景确定性验证：无损 PITR、时间点恢复、缺 WAL 快速失败、迁移前逻辑备份、SIGKILL 硬崩溃故障注入、流式备库优雅停机/追赶/故障切换、网络分区隔离/重连追赶、归档目标故障/积压排空/无损 PITR；连续两轮运行一致，报告见 `.artifacts/wal-pitr-drill/`，记录见 `docs/changes/2026-08-12-m101-wal-pitr-drill.md`）。真实磁盘压力（ENOSPC）与跨主机 HA 依赖组织授权（M90 授权轨），保持 Deferred。
...
**进度（2026-08-12，本地轨道 v4）**：M102 双全新环境安装/升级/回滚/备份恢复演练完成（`scripts/dual-env-compose-drill.sh`，两套完全隔离环境 install/关键旅程/数据持久化/清理；`APP_UPGRADE_BACKEND_IMAGE` 补跨 digest 升级/回滚；`APP_BACKUP_RESTORE=1` 补第三全新环境逻辑备份恢复，16/16 PASS，报告见 `.artifacts/dual-env-compose-drill/`，记录见 `docs/changes/2026-08-12-m102-dual-env-compose-drill.md` v3）；**可复现闭环演示完成**（`scripts/demo-drill.sh` + 仓库内 mock Kubernetes API `backend/cmd/demo-kube-mock`，隔离离线环境驱动平台真实 API 串起「登录→态势→根因→证据→受控动作→验证→事故复盘」15/15 PASS，报告见 `.artifacts/demo-drill/`，记录见 `docs/changes/2026-08-12-m102-demo-drill.md`）；**M102 最终交付文档完成**（测试矩阵 `docs/testing/test-matrix.md`、已知限制 `docs/testing/known-limitations.md`、兼容范围 `docs/testing/compatibility.md`、运维手册 `docs/operations/runbook.md`、安全声明 `docs/security/security-statement.md`，记录见 `docs/changes/2026-08-12-m102-final-docs.md`）；**离线安装包全链路演练完成**（`scripts/offline-install-drill.sh`，bundle→SHA256 校验→docker load→`pull_policy: never` 全新环境安装→关键旅程→清理，10/10 PASS 连续多轮一致，并发布可复用离线包 `.artifacts/offline-install-drill/bundle/`，记录见 `docs/changes/2026-08-12-m102-offline-install-drill.md`）。真实组织 kind/Helm 生命周期、WAL/PITR 与 M89/M90 授权轨仍需组织环境补齐；未完成前版本保持 RC，不宣称 GA。
...
5. M96 Hosted Gate B 与 M97 Gate C 已由 `v0.3.0-rc.4` 证据关闭；M98 事故工作空间已于 2026-08-12 交付，M99 信号关联与 SLO 已于 2026-08-12 封口；M100-A（权限矩阵 + AIOps 授权强制）已于 2026-08-12 交付，继续推进 M100-B/C/D 安全与租户治理。
6. M89/M90 从现在开始准备授权与环境，但其完成状态独立于本地主线，最终只在 Gate D 汇合。
   - 授权准备材料已就绪：`docs/authorization-gate-prep.md`（最小授权包、验收范围、本地证据引用、授权后执行顺序）。

...
**进度（2026-08-12，本地轨道 v3）**：M102 双全新环境安装/升级/回滚/备份恢复演练完成（`scripts/dual-env-compose-drill.sh`，两套完全隔离环境 install/关键旅程/数据持久化/清理；`APP_UPGRADE_BACKEND_IMAGE` 补跨 digest 升级/回滚；`APP_BACKUP_RESTORE=1` 补第三全新环境逻辑备份恢复，16/16 PASS，报告见 `.artifacts/dual-env-compose-drill/`，记录见 `docs/changes/2026-08-12-m102-dual-env-compose-drill.md` v3）。真实组织 kind/Helm 生命周期、WAL/PITR 与 M89/M90 授权轨仍需组织环境补齐；未完成前版本保持 RC，不宣称 GA。

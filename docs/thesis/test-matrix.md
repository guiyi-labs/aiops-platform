...
| M20 Phase 8 PostgreSQL 恢复演练 | `.artifacts/postgres-recovery/postgres-recovery-20260728-124206.json`、`docs/changes/2026-07-28-postgres-backup-restore.md` | 本地通过；PostgreSQL 17 源实例应用 15 个迁移并写入身份/RBAC/集群凭据/诊断/审计/筛选器合成数据，81,265 字节 custom dump 导出后源实例先销毁，再在全新实例恢复；迁移和表级计数一致、外键异常为 0，容器/匿名卷/临时备份/进程凭据清理通过；托管 CI 待确认 |

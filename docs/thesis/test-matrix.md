...
| M20 Phase 11 OIDC/MFA 就绪门禁 | `.artifacts/identity-readiness/identity-readiness-20260728-165405.json`、`.artifacts/verification/verify-20260728-165939.json`、`docs/changes/2026-07-28-identity-readiness-gate.md` | 本地通过；离线严格解析 policy/discovery/JWKS，14 项检查覆盖 HTTPS issuer/endpoint、Code + PKCE S256、scope/签名/JWKS、claim/MFA、不可变 subject 绑定、session/logout 与 break-glass；无网络演练拒绝 issuer/PKCE 和 MFA/email-linking 降级且清理完整；300.97 秒全量门禁含 171 个 Go 测试入口、四个后端构建目标、14/59 前端测试、三服务 healthy 与 Kustomize 16/5/22/3；不声明生产 SSO 已启用，托管 CI 待验收 |

...
| M21 Phase 3 鉴权精确序列历史 API | `.artifacts/metrics-history-e2e/metrics-history-e2e-20260729-081759.json`、`.artifacts/verification/verify-20260729-082024.json`、`docs/adr/0036-authenticated-exact-series-metrics-history.md` | 本地验收通过；真实 PostgreSQL E2E 验证跨集群/跨序列隔离、两个有序点、三次采集覆盖、一个稀疏缺样本、后端重启持久性与夹具级联清理；115.83 秒完整门禁含 Go vet、199 个 Go 测试入口、五个后端构建目标、14 个 Vitest 文件/59 个用例、前端生产构建、前后端 Docker 镜像、三服务 healthy、Kustomize 16/5/22/3 和直连/代理 readiness；实现 revision 与 Hosted CI 待推送后补录 |
